...
class WeeklyResampleTests(unittest.TestCase):
    def test_includes_completed_friday_week_for_friday_anchored_weeks(self) -> None:
        daily = pd.DataFrame(
            [
        ...
        self.assertEqual(float(weekly.iloc[-1]["close"]), 108.0)

    def test_excludes_current_incomplete_friday_week_on_monday(self) -> None:
        daily = pd.DataFrame(
            [
                {"date": "2026-04-17", "open": 100, "high": 110, "low": 99, "close": 108, "volume": 1000},
                {"date": "2026-04-20", "open": 109, "high": 112, "low": 107, "close": 111, "volume": 1200},
